    ...
    <div class="main-footer" style="display: flex; flex-direction: column; background: color-mix(in srgb, var(--background-color) 85%, #1c1726) !important; color:var(--text-color); padding: 4rem 10%; font-family:'Outfit', sans-serif;">
        ...
        <div style="display:flex; justify-content:space-between; gap: 2rem;" class="footer-grid">
            <div class="footer-col" style="flex: 1;">
                ...
                <p style="margin-bottom:0.3rem;"><strong style="color:var(--text-color); font-weight:600;">Address:</strong></p>
                ...
                <p style="margin-bottom:0.3rem;"><strong style="color:var(--text-color); font-weight:600;">Contact:</strong></p>
                <p style="margin-top:0; margin-bottom:1rem; font-size:0.9rem;">(700) 555-0199</p>
                <p style="margin-bottom:0.3rem;"><strong style="color:var(--text-color); font-weight:600;">E-mail:</strong></p>
...
# PAGE 1: LIVE TRIAGE & ALERTING
    ...
    st.markdown('''<style>
.stApp::before { content: ''; position: fixed; top: 0; left: 0; width: 100vw; height: 100vw; background: radial-gradient(circle at 80% 20%, rgba(255, 61, 0, 0.15) 0%, transparent 40%); z-index: 0; animation: pulseAlert 4s infinite alternate; pointer-events: none; }
.stApp::after { content: ''; position: fixed; bottom: 0; left: 10%; width: 60vw; height: 60vw; background: radial-gradient(circle, rgba(116, 50, 255, 0.1) 0%, transparent 50%); z-index: 0; animation: driftBg1 15s infinite alternate ease-in-out; pointer-events: none; }
...
# PAGE 2: LIVE ACCELERATOR BENCHMARK
    ...
    st.markdown('''<style>
.stApp::before { content: ''; position: fixed; top: -20%; left: -10%; width: 50vw; height: 50vw; background: radial-gradient(circle, rgba(0, 230, 118, 0.15) 0%, transparent 60%); z-index: 0; animation: speedFloat 8s infinite alternate ease-in-out; pointer-events: none; }
.stApp::after { content: ''; position: fixed; bottom: -10%; right: -20%; width: 70vw; height: 70vw; background: radial-gradient(circle, rgba(0, 210, 255, 0.1) 0%, transparent 50%); z-index: 0; animation: speedFloat 6s infinite alternate-reverse ease-in-out; pointer-events: none; }
...
# PAGE 3: TRANSACTION RISK SIMULATOR
    ...
    st.markdown('''<style>
.stApp::before { content: ''; position: fixed; top: 30%; right: 10%; width: 40vw; height: 40vw; background: radial-gradient(circle, rgba(255, 145, 0, 0.12) 0%, transparent 60%); z-index: 0; animation: rotateGlow 20s linear infinite; pointer-events: none; }
.stApp::after { content: ''; position: fixed; bottom: 10%; left: -10%; width: 50vw; height: 50vw; background: radial-gradient(circle, rgba(75, 54, 124, 0.2) 0%, transparent 60%); z-index: 0; animation: driftBg2 15s infinite alternate ease-in-out; pointer-events: none; }
...
# PAGE 4: GEMINI FRAUD COPILOT
    ...
    st.markdown('''<style>
.stApp::before { content: ''; position: fixed; top: -10%; left: 20%; width: 60vw; height: 60vw; background: radial-gradient(circle, rgba(66, 133, 244, 0.12) 0%, transparent 50%); z-index: 0; animation: morphAI 15s infinite alternate ease-in-out; pointer-events: none; }
.stApp::after { content: ''; position: fixed; bottom: -20%; right: 10%; width: 70vw; height: 70vw; background: radial-gradient(circle, rgba(234, 67, 53, 0.08) 0%, transparent 50%); z-index: 0; animation: morphAI2 18s infinite alternate-reverse ease-in-out; pointer-events: none; }
...
# PAGE 5: GCP ENTERPRISE ARCHITECTURE
    ...
    st.title("Google Cloud Data & Application Stack")
    st.markdown('''<style>
.stApp::before { content: ''; position: fixed; top: 10%; left: -20%; width: 140vw; height: 30vh; background: radial-gradient(ellipse, rgba(142, 197, 252, 0.08) 0%, transparent 70%); z-index: 0; animation: cloudDrift 30s infinite linear; pointer-events: none; }
.stApp::after { content: ''; position: fixed; bottom: 20%; right: -20%; width: 120vw; height: 40vh; background: radial-gradient(ellipse, rgba(224, 195, 252, 0.06) 0%, transparent 60%); z-index: 0; animation: cloudDrift2 40s infinite linear reverse; pointer-events: none; }
